...
object UserDaoDb extends UserDaoLike {
  import ctx._
  ...
  def saveUser(userName: String, password: String): ObjectId = {
    ...
    val q = quote { query[Users].insert(lift(Users(oid, userName, password))) }
    run(q)
    oid
  }
  ...
  def checkUserExist(username: String): Option[Users] = {
    val q = quote { query[Users].filter(user => user.userName == lift(username)).take(1) }
    run(q).headOption
  }
}
